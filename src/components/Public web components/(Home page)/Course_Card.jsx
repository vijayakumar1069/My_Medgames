...
const Course_Card = ({ course }) => {
  const { _id, name, description, img_for_home, price, startDate, endDate, via, dailyStartTime, dailyEndTime, classDays } = course;

  return (
    <div className="group max-w-2xl w-full bg-[#F4F6FC] rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
      <div className="relative w-full aspect-video overflow-hidden">
        <Image src={img_for_home} alt={name} fill priority quality={90} className="object-cover object-center transition-transform duration-500 group-hover:scale-110" sizes="(max-width: 640px) 100vw, (max-width: 768px) 50vw, (max-width: 1024px) 33vw, 400px" />
